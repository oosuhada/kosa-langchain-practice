...
// 텍스트 대화와 관련된 코드
// ############################################################################
window.ai_utils = window.ai_utils || {};

// ##### 사용자 질문을 보여줄 엘리먼트를 채팅 패널에 추가하는 함수 #####
ai_utils.addUserQuestion = function(question, chatPanelId) {
	const html = `
    <div class="d-flex justify-content-end m-2">
      <table>
        <tr>
          <td><img src="/static/common/image/user.png" width="30"/></td>
  ...
  `;
	...
	ai_utils.scrollToHeight(chatPanelId);
};

// ##### 응답을 보여줄 엘리먼트를 채팅 패널에 추가하는 함수 #####
ai_utils.addAnswerPlaceHolder = function(chatPanelId) {
    ...
    <div class="d-flex justify-content-start border-bottom m-2">
      <table>
        <tr>
          <td><img src="/static/common/image/assistant.png" width="50"/></td>
...
// ##### 텍스트 응답을 출력하는 함수 #####
// ai_utils.printAnswerText = async function(responseBody, targetId, chatPanelId) {
// 	ai_utils.printAnswerStreamText(responseBody, targetId, chatPanelId);
// }

ai_utils.printAnswerText = async function(response, targetId, chatPanelId) {
	...
	if (contentType.includes('text/plain')) {
		// text/plain 응답 처리 (일반 텍스트)
		const text = await response.text();
		const targetElement = document.getElementById(targetId);
		const htmlContent = marked.parse(text);
		const styledContent = ai_utils.applyBootstrapStyles(htmlContent);
		targetElement.innerHTML = styledContent;
		ai_utils.scrollToHeight(chatPanelId);
	} else if(contentType.includes('application/x-ndjson')) {
		// application/x-ndjson 응답 처리 (스트리밍)
		ai_utils.printAnswerStreamText(response.body, targetId, chatPanelId);
	} else if(contentType.includes('application/json')) {
		const jsonString = await response.text();
        ai_utils.printAnswerJson(jsonString, targetId, "chatPanel");
	}
}

// ##### JSON을 이쁘게 출력하는 함수 #####
ai_utils.printAnswerJson = async function(jsonString, uuid, chatPanelId) {
...
ai_utils.printAnswerStreamText = async function(responseBody, targetId, chatPanelId) {
	const targetElement = document.getElementById(targetId);
	...
	while (true) {
		...
		content += chunk;
		if (!ai_utils.isOpenTagIncomplete(chunk)) {
...
ai_utils.applyBootstrapStyles = function(html) {
	// 임시 DOM 생성하여 HTML 조작
...
ai_utils.isOpenTagIncomplete = function(str) {
...
ai_utils.scrollToHeight = function(chatPanelId) {
...
// ##### 진행중임을 표시하는 함수 #####
ai_utils.setSpinner = function(spinnerId, status) {
	if (status) {
...
// 음성 대화와 관련된 코드
// ############################################################################
window.ai_utils.voice = window.ai_utils.voice || {};

// ##### 마이크를 활성화하고 소리 분석 도구 및 녹화 도구를 준비를 하는 함수 #####
ai_utils.voice.initMic = async function(handleVoice) {
	//전역 변수 초기화
	ai_utils.voice.voice = false;              	// 사람의 음성이 입력되면 true 
	ai_utils.voice.chatting = false;           	// 질문하기 시작할 때부터 답변을 받을 때까지 true         
	ai_utils.voice.silenceStart = null;        	// 침묵 시작 시간을 저장
	ai_utils.voice.silenceDelay = 2000;      		// 침묵 지연 시간 2초을 저장하는 상수
	ai_utils.voice.silenceThreshold = 0.01;  		// 침묵인지 판단할 임계상수(0~1 사이의 값) 
	ai_utils.voice.stream = null;               // 마이크 입력 스트림 객체
	ai_utils.voice.analyser = null;             // 소리 분석기 객체
	ai_utils.voice.mediaRecorder = null;        // 음성 녹음기 객체
	ai_utils.voice.recognition = null;          // 음성 인식 객체

	//사용자에게 마이크 접근 권한을 요청하고, 오디오 스트림(MediaStream)을 가져옴
	const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
	ai_utils.voice.stream = stream;
	...
	ai_utils.voice.analyser = audioContext.createAnalyser();
	//음성 분석을 위한 FFT(빠른 푸리에 변환) 구간 크기 설정
	//클수록 더 정밀한 주파수 분석이 가능하지만 처리 비용이 증가(보통 512, 1024, 2048 사용)
	ai_utils.voice.analyser.fftSize = 2048;
	//오디오 소스를 분석기에 연결
	source.connect(ai_utils.voice.analyser);
	//-----------------------------------------------

	//미디어 녹음기 초기화
	ai_utils.voice.initMediaRecorder(handleVoice);
	//음성 인식 초기화
	ai_utils.voice.initRecognitionVoice();
};

//##### 미디어 녹음기를 초기화하는 함수 #####
ai_utils.voice.initMediaRecorder = function(handleVoice) {
	//오디오 녹음을 위한 MediaRecorder 생성
	const mediaRecorder = new MediaRecorder(ai_utils.voice.stream);
	ai_utils.voice.mediaRecorder = mediaRecorder;

	//침묵으로 인한 음성 녹화가 중지되었을 때, 자동 호출되는 함수 지정
	mediaRecorder.ondataavailable = async (event) => {
		//음성 확인이 되었고, 녹화 데이터가 있고, 현재 대화중이 아닐 경우
		if (ai_utils.voice.voice === true && event.data.size > 0 && ai_utils.voice.chatting === false) {
			console.log("대화 시작");
			ai_utils.voice.chatting = true;

			//MP3로 변환
			const webmBlob = event.data;
			const mp3Blob = await ai_utils.voice.convertWebMToMP3(webmBlob);
			//콜백(사용자 로직) 실행 -------------
			handleVoice(mp3Blob);
			//---------------------------------
		}
...
ai_utils.voice.initRecognitionVoice = function() {
	// 음성 인식 전역 변수 초기화
	ai_utils.voice.voice = false;
	// 음성 인식을 제공하는 SpeechRecognition 생성
	const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
	const recognition = new SpeechRecognition();
	ai_utils.voice.recognition = recognition;
	...
	recognition.onresult = function(event) {
		...
		if (transcript.length > 0 && ai_utils.voice.isKorean(transcript)) {
			console.log("한국어 음성 확인");
			ai_utils.voice.voice = true;
		}
	};
	// 인식을 종료할 때 콜백되는 함수
	recognition.onend = function() {
		// 브라우저에서 자동 종료시켰을 경우, 재시작 시킴
		if (!ai_utils.voice.voice) {
			recognition.start();
		}
	};
...
ai_utils.voice.isKorean = function(text) {
...
// ##### 침묵이 지속되는지 체크하는 함수 #####
ai_utils.voice.checkSilence = function() {
	// 분석 결과를 저장할 바이트 배열을 생성
	const dataArray = new Uint8Array(ai_utils.voice.analyser.fftSize);
	// 오디오 파형 데이터를 dataArray에 복사
	// 각 값은 0~255 범위의 8비트 정수이며, 오디오 신호의 진폭을 나타냄
	// 128이 중심(0에 해당), 0 또는 255는 최대 음파 진폭
	ai_utils.voice.analyser.getByteTimeDomainData(dataArray);
	...
	if (rms < ai_utils.voice.silenceThreshold) {
		// 침묵 시작 시간 설정이 되어 있지 않은 경우
		if (!ai_utils.voice.silenceStart) {
			// 침묵 시작 시간 설정
			ai_utils.voice.silenceStart = Date.now();
		}
		// 침묵이 silenceDelay 동안 지속될 경우
		else if ((Date.now() - ai_utils.voice.silenceStart) > ai_utils.voice.silenceDelay) {
			// 음성 녹화 중이라면, 음성 녹화 중지
			if (ai_utils.voice.mediaRecorder.state === 'recording') {
				ai_utils.voice.mediaRecorder.stop();
				ai_utils.voice.recognition.stop();
			}
			// 침묵 시작 시간 없애기
			ai_utils.voice.silenceStart = null;
			return;
		}
	}
	...
	requestAnimationFrame(ai_utils.voice.checkSilence);
};

// ##### WebM Blob을 MP3 Blob으로 변환 #####
// OpenAi의 gpt-4o-mini-audio의 입력은 audio/mp3 또는 audio/wav만 가능
ai_utils.voice.convertWebMToMP3 = async function(webmBlob) {
...
ai_utils.voice.playAudioFormStreamingData = async function(response, audioPlayer) {
	try {
...
ai_utils.voice.addUserQuestionPlaceHolder = function(chatPanelId) {
	//id-를 붙이는 이유: 숫자로 시작하면 CSS 선택자 문법 에러 날 수 있음
	let uuid = "id-" + crypto.randomUUID();
	const questionHtml = `
    <div class="d-flex justify-content-end m-2">            
        ...
        <tr>
          ...
          <td>
            <div id="${uuid}-speaker" class="speakerPulse" 
              ...
              background: url('/static/common/image/speaker-yellow.png') no-repeat center center / contain;"></div>
          </td>
          <td><span id="${uuid}"></span></td>
        </tr>
      </table>                      
...
ai_utils.voice.addAnswerPlaceHolder = function(chatPanelId) {
	//id-를 붙이는 이유: 숫자로 시작하면 CSS 선택자 문법 에러 날 수 있음
	let uuid = "id-" + crypto.randomUUID();
	let answerHtml = `
    <div class="d-flex justify-content-start border-bottom m-2">         
      <table>
        <tr>
          ...
          <td>
            <div id="${uuid}-speaker" class="speakerPulse" 
              style="width: 30px; height: 30px; 
              background: url('/static/common/image/speaker-green.png') no-repeat center center / contain;"></div>
          </td>
          <td><span id="${uuid}"></span></td>
        </tr>
      </table>            
...
// ##### 스피커 애니메이션 제어 함수 #####
ai_utils.voice.controlSpeakerAnimation = function(speackerId, flag) {
	if (flag) {
...
// 비전 및 이미지 생성
// ############################################################################
window.ai_utils.vision = window.ai_utils.vision || {};

// ##### 카메라 영상을 보여주는 함수 #####
ai_utils.vision.previewCamera = function(videoId) {
	//<video> 엘리먼트 얻기
...
ai_utils.vision.captureFrame = function(videoId, handleFrame) {
	//<video> 엘리먼트 얻기
	const video = document.getElementById(videoId);
...
ai_utils.vision.selectArea = { x: 0, y: 0, w: 0, h: 0 };

// ##### 마스트 투명 영역이 설정되었는지 체크하는 함수 #####
ai_utils.vision.existTransparentArea = function() {
	if (ai_utils.vision.selectArea.w === 0 && ai_utils.vision.selectArea.h === 0) {
		return false;
	} else {
		return true;
	}
...
// ##### 마스트 투명 영역을 제거하는 함수 #####
ai_utils.vision.clearTransparentArea = function() {
	ai_utils.vision.selectArea = { x: 0, y: 0, w: 0, h: 0 };
}

// ##### 캔버스 초기화 함수 #####
ai_utils.vision.initCanvas = function(imgId, canvasId) {
	// <img>에서 이미지가 로드 완료되었을 때 콜백되는 함수 등록
	// resizeCanvas(): <img>의 크기와 동일한 <canvas>를 만듬
	const img = document.getElementById(imgId);
	img.addEventListener("load", (e) => { ai_utils.vision.resizeCanvas(imgId, canvasId); });

	// window 크기가 변경되었을 때 콜백되는 함수 등록
	// resizeCanvas(): <img>의 크기와 동일한 <canvas>를 만듬
	window.addEventListener("resize", (e) => { ai_utils.vision.resizeCanvas(imgId, canvasId); });
	...
	canvas.addEventListener("mousemove", (e) => {
		...
		ai_utils.vision.selectArea = { x, y, w, h };
	...
	canvas.addEventListener("mouseup", e => {
		...
		// 투명 영역 콘솔에 출력해보기
		console.log("투명 영역:", ai_utils.vision.selectArea);
	});
};

// ##### 캔버스 크기 조정 함수 #####
ai_utils.vision.resizeCanvas = function(imgId, canvasId) {
	// <img>와 <canvas> 가져오기
	const img = document.getElementById(imgId);
	const canvas = document.getElementById(canvasId);
...
// ##### 원본 이미지 미리 보여주는 함수 #####
ai_utils.vision.previewImage = function(fileId, chatPanelId) {
...
ai_utils.vision.previewImage2 = function(fileId, imgId) {
	const file = document.getElementById(fileId).files[0];
	if (!file) return;
	const reader = new FileReader();
	reader.onload = e => {
...
// ##### 원본 이미지 Blob 얻기
ai_utils.vision.getOriginalBlob = function(imgId, fileId) {
	const img = document.getElementById(imgId);
...
// ##### 마스크 이미지 Blob 생성 함수 #####
ai_utils.vision.getMaskBlob = function(canvasId) {
	const canvas = document.getElementById(canvasId);
	...
	// 선택 영역만 투명
	const x = Math.round(ai_utils.vision.selectArea.x);
	const y = Math.round(ai_utils.vision.selectArea.y);
	const w = Math.round(ai_utils.vision.selectArea.w);
	const h = Math.round(ai_utils.vision.selectArea.h);
...
// ##### 마스크 이미지 다운로드 함수 #####
ai_utils.vision.downloadMaskImage = function(blob) {
...
ai_utils.vision.downloadBase64Image = function(base64Src, filename) {
	const link = document.createElement('a');
	link.href = base64Src;
...
// 이미지 임베딩과 관련된 코드
// ############################################################################
window.ai_utils.image = window.ai_utils.image || {};

// ##### 멀티 이미지 미리 보여주기 #####
ai_utils.image.previewMultiImages = function(fileId, chatPanelId) {
